...
Applications that already speak SMTP can hand mail to Lettr through its SMTP relay at `smtp.lettr.com`, where ports 465 and 2465 use implicit TLS and ports 25, 587 and 2587 use STARTTLS. The relay reads recipients from the SMTP envelope, so BCC addresses given there are delivered as regular recipients. The [How Email Delivery Works](https://docs.lettr.com/knowledge-base/fundamentals/how-email-delivery-works) page walks through each stage of the delivery pipeline.
