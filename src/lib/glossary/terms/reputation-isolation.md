...
Templates follow the same split. Every Lettr template belongs to one of two modes, **Transactional** for sending through the API or SMTP and **Marketing** for campaigns. API sends can also carry a `tag` of up to 64 characters to group related sends for analytics.

The Sending Reputation guide names domain reputation as the factor a sender controls most directly, and Lettr has no single reputation score to watch in its place. Dedicated IPs, for IP-level separation, are available as an add-on on the Business plan and on Enterprise. The [Subdomain vs Root Domain for Sending](https://docs.lettr.com/knowledge-base/fundamentals/subdomain-vs-root) page walks through the trade-offs.
