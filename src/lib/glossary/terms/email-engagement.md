...
Engagement reaches webhook endpoints as `engagement.open`, `engagement.initial_open` for the first open of an email by a recipient, and `engagement.click`. In the Events dashboard, first and later opens both show as opened badges and clicks as clicked badges. The Message Details view adds the user agent and a **Prefetched** field that marks opens triggered by a mail client prefetching images.
