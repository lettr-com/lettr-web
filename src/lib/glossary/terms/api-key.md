...
[Sandbox](/glossary/sandbox-mode/) keys redirect every message to the email address of the user who created the key, rewrite the From domain to the pre-verified `dev.uselettr.com` and have no billing impact, so they work before any sending domain is verified. They carry stricter [rate limits](/glossary/rate-limiting/) than live keys: 10 requests per minute and 100 per day.
