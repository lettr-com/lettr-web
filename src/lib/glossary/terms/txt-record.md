...
The verification endpoint, `POST /api/domains/{domain}/verify`, returns `spf_status` and `dmarc_status` next to `dkim_status` and `cname_status`. Verification typically takes a few minutes but can take up to 48 hours. BIMI uses a TXT record at `default._bimi` and is configured entirely through DNS, with no BIMI configuration in the Lettr dashboard or API.
