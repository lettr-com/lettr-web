...
On DNS providers that support Domain Connect, Lettr can add these records automatically, and on Cloudflare the CNAME records are created in DNS only mode. The [Sending Domains](https://docs.lettr.com/learn/domains/sending-domains) and [Tracking Domains](https://docs.lettr.com/learn/domains/tracking-domains) pages in the Lettr docs list each record.
