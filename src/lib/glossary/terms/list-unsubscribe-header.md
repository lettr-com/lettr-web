...
Neither header can be set through the `headers` field of the send API, which blocks `List-Unsubscribe` and `List-Unsubscribe-Post` along with other headers Lettr manages itself, such as `Message-ID` and `Return-Path`. API sends default to `transactional: true`, and that default bypasses unsubscribe suppression, so marketing-shaped API sends set `transactional: false` for unsubscribes to be honored. The [Complaints and Unsubscribes](https://docs.lettr.com/learn/suppressions/complaints-unsubscribes) page documents both unsubscribe events.
